...
function uniqWith(array, comparator) {
    return array.reduce((acc, d) = > !acc.some(item = > comparator(d, item)
)
        ? [...acc, d
] :
    acc, []
...
function getIcssReplacer(item, loaderContext, importPrefix, onlyLocals) {
    ...
    return () =
>
    onlyLocals ? `" + require(${(0, _loaderUtils.stringifyRequest)(loaderContext, importUrl)})[${JSON.stringify(item.export)}] + "` : `" + require(${(0, _loaderUtils.stringifyRequest)(loaderContext, importUrl)}).locals[${JSON.stringify(item.export)}] + "`;
}
...
function prepareCode(file, messages, loaderContext, importPrefix, onlyLocals) {
    ...
    messages.filter(message = > message.type === 'icss-import' || message.type === 'import' && message.importType === 'url'
).
    forEach(message = > {
